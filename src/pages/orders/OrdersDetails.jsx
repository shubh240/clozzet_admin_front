...
export default function OrderDetails() {
  ...
  const fetchOrderDetails = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/order/order-details/${orderId}`, {
        headers: {
  ...
  if (!order) {
    return <div className="container my-4 text-danger">Order not found or failed to load.</div>;
  }
  ...
  return (
    <div className="container my-4">
      <Button variant="secondary" className="mb-3" onClick={() => navigate('/order-list')}>
        ← Back to Orders
      </Button>
        ...
        <div className="d-flex justify-content-between mb-4">
          ...
          <div className="text-end">
            ...
            <div className="d-flex gap-2 flex-wrap justify-content-end">
                    <Badge bg={paymentStatus === 'Paid' ? 'success' : 'warning'}>
                    {paymentStatus}
                </Badge>
                {isCancelled && (
                    <Badge bg="danger">
        ...
        {shipment?.dropAddressLine1 && (
          <div className="mt-3">
            ...
            {shipment.dropAddressUrl && (
              <a 
              href={shipment.dropAddressUrl} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="btn btn-sm btn-outline-primary"
              style={{ width: 'fit-content', padding: '4px 10px', fontSize: '14px' }}
            >
                View on Map
              </a>
            )}
        ...
        {shipment?.pickupAddress && (
          <div className="mt-3">
            ...
            {shipment.pickupAddressUrl && (
              <a 
              href={shipment.pickupAddressUrl} 
              target="_blank" 
              rel="noopener noreferrer" 
